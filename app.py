...
from models import db, connect_db, User, Post, PostTag, Tag #added imports 10/9/23 10:45AM remove to return function
...
def add_post_form(user_id):
    user = User.query.get_or_404(user_id)
    tags = Tag.query.all()
    #tags = Tag.query.all() to query all tags?
    #tag_list = request.form['tags']  to capture the clicked tags?


    return render_template("add_post_form.html", user=user, user_id=user_id, tags=tags)  #needed user_id=user_id

#editing 10/9 12:51pm to do post and get
@app.route('/create_post/<int:user_id>', methods=['GET', 'POST'])
def create_post(user_id):
    if request.method == 'POST':
        title = request.form['post_title']
        content = request.form['post_content']
        selected_tag_ids = request.form.getlist('tag_checkbox')
    ...
    # Redirect to the post_detail page for the newly created post
        return redirect(url_for('post_detail', user_id=user_id, post_id=new_post.id))
    
    tags = Tag.query.all()
    return render_template('add_post_form.html', user_id=user_id, tags=tags)
...
def post_detail(user_id, post_id):
     ...
     user = User.query.get_or_404(user_id)
     #required a join...
     post_tags = Tag.query.join(PostTag).filter(PostTag.post_id == post_id).all()
     ...
     return render_template("post_detail.html", post=post, user=user, user_id=user_id, post_id=post_id, post_tags=post_tags)
...
def post_edit(user_id, post_id):
    ...
    #Seems that 
     if request.method == 'POST':
         ...
         #update(posts).values(new_title=new_title, new_content=new_content)
         db.session.commit()
...
#added 10/9/23 11:11AM.  This works and added the tag to the database.
@app.route('/tag_add/<int:user_id>', methods=['POST'])
def add_tag(user_id):

#remember, this tag should NOT have a post ID associated
#CORRECT THIS SO A 404 RETURNS NOT SQL ERROR FOR DUPLICATE TAG ID
    #if request.method == 'POST':
    #get the tag text from the input box
    ...
    #tags = Tag.query.all()  add to tag_list?
    return redirect(url_for('display_user_details', user_id=user_id))
    #return render_template("tag_list.html", tags=tags)

#this lists all the tags with checkboxes so far 10/9 2:40PM
@app.route('/tag_list')
def list_tags():
    tags = Tag.query.all()
    #need to add href's in tag_list.html
